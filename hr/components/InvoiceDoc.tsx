// Invoice document — renders as a printable A4 sheet using the same letterhead
// CSS used for offer letters and payslips. Imported by the view page and the
// PDF generation route so the two can never drift.
    ...
    <html lang="en">
      <head>
        ...
        <style dangerouslySetInnerHTML={{ __html: LETTERHEAD_CSS + `
          @media print {
            .ss-stage { background: #fff; padding: 0; }
            .ss-sheet { box-shadow: none; max-width: 100%; padding: 20px 30px; }
      ...
      <body style={{ margin: 0, padding: 0, background: "#6b7280" }}>
        <div className="ss-stage">
          <div className="ss-sheet" style={{ position: "relative" }}>
            ...
            <div className="ss-mast">
              {logoDataUri && (
                <img src={logoDataUri} alt="" className="ss-logo" />
              )}
              <div>
              ...
              <table style={{ width: 280, borderCollapse: "collapse" }}>
                <tbody>
                  ...
                  {gstAmount > 0 && invoice.gstRate > 0 && (
                    <Row label={`GST @ ${invoice.gstRate}%`} value={formatAmount(gstAmount)} />
                  )}
                  <tr>
                    <td colSpan={2} style={{ borderTop: "2px solid #1f3a5f", paddingTop: 6 }} />
                  </tr>
                  <Row label="Total Due" value={formatAmount(total)} bold large />
                </tbody>
              </table>
            </div>
